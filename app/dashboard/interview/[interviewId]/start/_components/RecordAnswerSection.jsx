    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const result = await model.generateContent([
          "Transcribe the following audio and detect if multiple voices are present. If there are multiple voices, return a warning.",
          { inlineData: { data: base64Audio, mimeType: "audio/webm" } },
        ]);

        const transcription = result.response.text();

        // Check for multiple voice detection warning
        if (transcription.includes("multiple voices")) {
          toast.warning("Warning: Multiple voices detected!");
        }
  ...
  const updateUserAnswer = async () => {
    try {
      ...
      const feedbackPrompt =
        ...
        ", User Answer:" +
        userAnswer +
        " , Please give a rating and feedback in JSON format with 'rating' and 'feedback' fields.";
  ...
  return (
    <div className="flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="md:flex mt-4 md:mt-8 md:gap-5">
        ...
        <Button variant="outline" onClick={isRecording ? stopRecording : startRecording} disabled={loading}>
          {isRecording ? (
